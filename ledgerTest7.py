        ...
        for packet in exchange_packet:
            if (random.randrange(0, 100)) > ComErrorRatio:  # Communication Error
                if (random.randrange(0, 100)) < dataErrorRatio:  # Data Error
                    if 'newTransaction' in packet:
                        errpacket = copy.copy(packet)
                        errpacket['newTransaction'] = []
                        for transaction in packet['newTransaction']:
                            errpacket['newTransaction'].append(copy.copy(transaction))
                        errpacket['newTransaction'][0]['balance'] = random.randrange(0, 1000)
                        print("packet = %s" % packet)
                        print("new packet = %s" % errpacket)
                        ReceivedPackets.append(errpacket)
                else:
                    ReceivedPackets.append(packet)
