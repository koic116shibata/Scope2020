...
node_core2 = node_core_M.node_core('bbbbbb')
print("\n\nnode2 status = %s" % node_core2.get_request_status())
node_core2.create_new_ledger()
print("\n\nnode2 status = %s" % node_core2.get_request_status())

account_list = node_core2.get_account_list()
print("account list: %s" % account_list)

print(node_core2.get_ledger_ID_list())
print(node_core.get_ledger_ID_list())

ledgerlist = node_core.get_ledger_ID_list()
ledger_content = node_core.get_ledger_contents(ledgerlist[0])
print(ledger_content)

node_core2.create_clone_ledger(ledger_content)
ledgerlist = node_core2.get_ledger_ID_list()
print(ledgerlist)
ledger_content = node_core2.get_ledger_contents(ledgerlist[1])
print(ledger_content)
